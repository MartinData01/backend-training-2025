...
// 請文字解釋為什麼
console.log(k * l > 21);
// 變數相乘時，JS會嘗試將值轉換成數字再執行相乘，因 l 是字串8，因此轉型成數字8後與數字3相乘，結果為數字24，大於數字21
...
if (BobPrice > giftPriceRule || BobIsVip || giftNum > 0) {
...
if (playerA === playerB) {
  ...
  console.log('playerA 贏了！');
}else if (playerB === '剪刀' && playerA === '布'){
  console.log('playerB 贏了！');
}else if(playerB === '石頭' && playerA === '剪刀'){
  console.log('playerB 贏了！');
}else if(playerB === '布' && playerA === '石頭'){
  console.log('playerB 贏了！');
}else{
  console.log('錯誤')
}
...
const gymCoach = {
  name: '高雄市健身教練聯盟',
  location: '高雄市',
  description: '這是一間位於高雄市的健身房，名為「高雄市健身教練聯盟」，專注於提供高品質的健身指導服務。',
  coaches:[
    {
      name: '王教練',
      expertise: '力量訓練、減重課程',
...
// 練習：計算小明今日消耗的卡路里
calorieBurn += activities.bike.calories*10
